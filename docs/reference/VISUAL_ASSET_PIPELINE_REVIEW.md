...
|---|---|---|---|
| **T1, Format** | The [glTF 2.0 specification](https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html) and the Khronos extension registry (which documents official extentions to glTF) | We currently do not do anything official, but Carlos review revealed a number of potential (but unverified) inconsistencies (we work thorugh those below).  See [ASSET_AUDIT_PARTS_LIBRARY.md](ASSET_AUDIT_PARTS_LIBRARY.md) and [ASSET_AUDIT_BLUEBOAT_CHASSIS.md](ASSET_AUDIT_BLUEBOAT_CHASSIS.md).  | A few things: Validate against Honu's [model-spec.md](model-spec.md), which embodies the narrow scope of glTF spec applied to models for this project.; Validate against [Khronos glTF Validator](https://github.com/KhronosGroup/glTF-Validator), % CLAUDE: Should this be manual, done via agents or done in CI? Evidence tag V |
| **T2, Appearance** | The result of a conformant rending tool, checked against the agreed upon level of visual detail.  | Because we have not specified a fully conformant glTF renderer, the authors and reviewers are using mulitiple tools which each have their own results that are not necessarily consistent.  The modeler looks at Blender's viewport, which shows Blender's materials rather than the exported file. Nobody views the delivered file in a reference renderer | The modeler opens the exported file in the Khronos glTF Sample Viewer % CLAUDE: provide link
and signs it off before delivery. Blender's viewport does not count. Evidence tag R |
...
% CLAUDE: We need to extend this or include in the existing items file size.  If we are going to stick with an all in one GLB file, we need to assess the number of vertices in the mesh and the resolution of the texture images.   Have to think about how we folkd this into the table.   
