...
* Images either sit in the BIN chunk (referenced by `bufferView`) or are external files referenced by `uri`. % CLAUDE: Call out a decision here.   There will be a bunch of project-specfic guidelines, that build upon the spec, but constain for our use-case.   A decision here is if we want to say that images (textures) should be embedded, exteral or both.  Currently we use embedded, but from a git perspective, it might be better to have them disaggregated. I get the one file cleanliness, but it seems like if we want to iterate on only part of the asset, it is better to do so
...
* units are meters; 
* the coordinate system is right-handed, +Y up, and the front of an asset faces +Z; Note the "+Z forward" convention is what the spec says, and section 1.6 shows our files do not and must not follow it. % CLAUDE: It is very important that we get this "right" - by "right" i mean we are intentional and clear with what we are doing, expecially if we are goin gto violate the spec.  Is this portion of the spec applicable to each mesh within the glTF tree?  Does it specify the original location and orientation of the origin frame for each mesh (the geometry reference)?  
* UV origin is the top-left of the image - % CLAUDE: What is UV?
* base color and emissive are sRGB, every other map is linear; % CLAUDE: explain
...
Read out of the drydock container on 2026-09-04. Nothing in this project pins them: the Gazebo libraries are whatever the `ros-lyrical-gz-*-vendor` packages (the Jetty line) resolve to on Ubuntu 26.04, and assimp is the Ubuntu package. An `apt upgrade` or an image rebuild can move any row, and every S-tagged claim below is tied to these numbers. % CLAUDE: We should discuss how much resolution we need to maintain on this versioning.  I had one recent experience with an older version of Gazebo where the way it handled GLB was very diffreent that the current method.   Can we definitively say that the way gazebo and rviz handles glTF/GLB will not change - i.e., is stable?   Seems like it could be unstable.
...
% CLAUDE: Wait this seems like more signs of instability on the Gazebo side.   Is this one of the key issues, that Gazebo has not implemented the complete spec and is in active development on this topic, so expecting Gazebo to be consistent with "truth" was not a good assumption?
...
6. "Author to the glTF spec: +Y up, +Z forward, no baked rotation nodes." Y up: Verified against T1 [D], and it is the whole reason `parts.xacro` and RViz agree. +Z forward: the draft quotes T1 correctly ("the front side of a glTF asset faces +Z"), and this project deliberately does not follow that sentence: forward is +X in every delivered file, because the pipeline maps ROS x to file X (section 1.6), and it must stay so. Class: Pipeline. Refuted as a rule for our modelers [F, S]; the spec text must state our convention and say explicitly that it overrides the glTF front-facing convention. Rotation nodes: the loader applies them [S], the constraint comes from `gltf_to_yup.py` [S], so the rule stands with a corrected reason and should say "transforms applied in Blender before export". "Do not fix the orientation for Gazebo yourself": Verified.
...
2. Mesh origin. Existing docs: at the mesh centroid. Draft spec: the part author's choice, declared in the macro comment. The macro handles either; the question is what a replacement mesh is authored to.
