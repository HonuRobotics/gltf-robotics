...
* Incorrect mental model equating classic mesh+texture models with glTF (PBR) modles.   
    - The current workflow was based on an incorrect idea that glb assets could be thought of as direct, drop-in replacement for a DAE Collada file referencing a paired PNG texture file
    - Consequently, we largely ignored that the [glTF 2.0 specification](https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html) covers a much larger scope of 3D asset use cases that required by this project.    The spec for glTF assets includes a large variety arrays (e.g., scene, node, camera, animation, skin, etc. ) that are not pertinent to this use case.   (The glTF spec is almost complex enough to be used as an SDF or URDF replacement.)  
    - For this project we must be intentional about what subset of functionality is being used so thta we (and our agents) don't naively apply the entire specification.  
* No single source of "truth" with respect to visual rendering
    ...
    - Because the tools we are using are inconsistnet, we each "seeing" different rendering behavior and each evaluating the models based on different criteria.
...
|---|---|---|---|
| **T1, Format** | The [glTF 2.0 specification](https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html) and the Khronos extension registry (which documents official extensions to glTF) | We currently do not do anything official, but Carlos review revealed a number of potential (but unverified) inconsistencies (we work through those below).  See [ASSET_AUDIT_PARTS_LIBRARY.md](ASSET_AUDIT_PARTS_LIBRARY.md) and [ASSET_AUDIT_BLUEBOAT_CHASSIS.md](ASSET_AUDIT_BLUEBOAT_CHASSIS.md).  | Two checks, in order. Validate against the [Khronos glTF Validator](https://github.com/KhronosGroup/glTF-Validator), zero errors, then against Honu's [model-spec.md](model-spec.md), which narrows the glTF spec to this project. Both are mechanical and deterministic, so both belong in CI beside the base color guard already in `test/test_parts.py`, with the workflow fetching the validator binary since it has no apt package. Not agents: a conformance gate has to give the same answer every time. Section 10, decision 7. Evidence tag V |
| **T2, Appearance** | The result of a conformant rendering tool, checked against the agreed upon level of visual detail.  | Because we have not specified a fully conformant glTF renderer, the authors and reviewers are using multiple tools which each have their own results that are not necessarily consistent.  The modeler looks at Blender's viewport, which shows Blender's materials rather than the exported file. Nobody views the delivered file in a reference renderer | The modeler opens the exported file in the [Khronos glTF Sample Viewer](https://github.khronos.org/glTF-Sample-Viewer-Release/) and signs it off before delivery. Blender's viewport does not count. The viewer runs the Khronos validator inline, so one drag and drop answers T1 and T2 together. Evidence tag R |
| **T3, Targets** | What our two consumers render: Gazebo (gz-sim 10, gz-rendering 10, Ogre-Next 2.3) and RViz (rviz_rendering 15.2), each implementing only a subset of T1 (sections 1.4 and 1.5) | Observation, on the versions we actually ship. Does the part render correctly  and load without error | `glb_probe` for what the loader built, which needs no GPU, then the parts world in Gazebo and the model in RViz. Evidence tags S and P |
| **T4, Budget** | No external standard. glTF imposes no limit on triangles, texture size or file size, and neither does Gazebo, so the only authority is Honu's [model-spec.md](model-spec.md) | Nothing is checked. Measured for this review: files from 15 KB to 2.85 MB, 4.8 MB in total; triangles from 164 to 21,776, 43,866 in total; 33 embedded images, the largest 2.1 MB, and textures are 59 percent of all bytes stored | Thresholds stated in [model-spec.md](model-spec.md) and enforced by the lint, once section 10, decisions 4 and 5 fix the numbers. Texture resolution is the lever that matters, not triangle count |
