...
- History in this repo
    - What this repo already decided: [`docs/design/parts.md`](../design/parts.md) (mesh conventions), [`docs/how-to/add-part.md`](../how-to/add-part.md) (the acceptance steps), the earlier README section "Accepting a new part" (commit `b5daead`, August 2026), and the sandbox findings carried over from the vrx4 branch.
...
* Incorrect mental model equating classic (mesh+image) pair with glTF models.   
    ...
    - Because glTF assets describe much more than the traditional (mesh, image) pair, we formulate [model-spec.md](model-spec.md) to describe both project conventions and constraints placed on models delivered in glTF format.
* No single source of "truth" with respect to visual rendering
    ...
    - Carlos is using Gazebo and other viewers such as F3D, as well as Claude introspection of the file contents against [`asset-spec.md`](asset-spec.md).
* Our consumers implement only a subset of glTF, so conforming (to glTF) and usable (in Gazebo and RVIZ) are different questions.
    ...
    - Because a valid glTF asset may not be usable in Gazebo or RViz, [model-spec.md](model-spec.md) adds constraints that come from our targets rather than from the format, and say plainly which rules are ours and which are the standard's.
...
|---|---|---|---|
| **T1, Format**<br>Is it valid glTF? | The [glTF 2.0 specification](https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html), the Khronos extension registry (which documents official extensions to glTF) and the project-specific [model-spec.md](model-spec.md)| Currently we do not have a reviewed, verified method for checking the format.  Carlos' previous review revealed a number of possible (but unverified) inconsistencies-see [ASSET_AUDIT_PARTS_LIBRARY.md](ASSET_AUDIT_PARTS_LIBRARY.md) and [ASSET_AUDIT_BLUEBOAT_CHASSIS.md](ASSET_AUDIT_BLUEBOAT_CHASSIS.md).  | Two checks: first validate against the [Khronos glTF Validator](https://github.com/KhronosGroup/glTF-Validator), then against Honu's [model-spec.md](model-spec.md), which narrows the glTF spec to this project. Both can be formulated deterministic go/no-go tests, so both belong in CI beside the base color guard already in `test/test_parts.py`, with the workflow fetching the validator binary since it has no apt package. **Not agents:** a conformance gate has to give the same answer every time. Section 10, decision 7. Evidence tag V |
...
| **T4, Budget**<br>Are the object sizes appropriate? | No external standard. glTF imposes no limit on triangles, texture size or file size, and neither does Gazebo, so the only authority is Honu's [model-spec.md](model-spec.md) | Nothing is checked. Measured for this review: files from 15 KB to 2.85 MB, 4.8 MB in total; triangles from 164 to 21,776, 43,866 in total; 33 embedded images, the largest 2.1 MB, and textures are 59 percent of all bytes stored | Thresholds stated in [model-spec.md](model-spec.md) and enforced by the lint, once section 10, decisions 4 and 5 fix the numbers. Texture resolution is the lever that matters, not triangle count |
...
The plan is therefore in two steps. Set provisional limits from judgment now, so a delivery has something to be checked against, and record them in [model-spec.md](model-spec.md) as guidelines with their basis stated as judgment rather than evidence. Then evaluate them against real performance when someone has time to measure, and revise the spec with what the measurement shows. Writing them into the spec is what makes the revision visible later; leaving them as habit would not.
...
**Origin: what we do now.** The written rule and the library disagree. [`docs/design/parts.md`](../design/parts.md) says the origin sits at the mesh centroid, the older README said the centroid and where the part would sensibly be mounted from, and the draft spec says it is the part author's choice. Measuring the delivered files against their own bounding boxes shows the library follows none of those three consistently, and instead follows a fourth rule nobody wrote down:
...
This section audits Carlos's draft as delivered, kept unmodified at [asset-spec.md](asset-spec.md). It has since been superseded by [model-spec.md](model-spec.md), and the findings below are what shaped it.
...
2. "`<part>.visual.glb`, next to `<part>.collision.stl`" and "Collision meshes are separate, simplified STL files." The visual name matches the repo convention. The collision statement does not: [`parts.md`](../design/parts.md) says collision is primitives stated in the macro, from the modeler's `model.sdf`, with a mesh only when a primitive will not do; one `.collision.stl` exists in the tree today against nine `model.sdf` files. Open. This is a pipeline decision (does the modeler still deliver `model.sdf`? do we accept STL envelopes?), and the spec must say which. See section 10, decision 1.
...
7. "Author the mesh in the part frame … Where the origin sits within the part is the part author's choice." Technically neutral: `attach` folds any origin into the mounting joint [S, `parts.xacro`]. It conflicts with [`parts.md`](../design/parts.md) rule 2, "origin at the mesh centroid", and the older README's "approximately at the centroid, in the place the part would sensibly be mounted from". Open. See section 10, decision 2.
...
The gap: [`parts.md`](../design/parts.md) says nothing about materials beyond "PBR materials embedded". Every material finding in this audit, the metalness defect included, has no home in the existing guidelines.

### 8.2 [`docs/how-to/add-part.md`](../how-to/add-part.md), the acceptance steps
...
Across [`parts.md`](../design/parts.md), the older README and the draft spec, the mesh origin is stated three ways: at the centroid, at the centroid and where the part would sensibly be mounted from, and wherever the part author chooses. The macro tolerates all three, so nothing has broken yet, and that is exactly why it has drifted. It needs one sentence in one place (section 10, decision 2).
...
11. Where the reconciliation in section 8 comes to rest. Either the draft spec absorbs the mesh conventions from [`parts.md`](../design/parts.md) and that file points at it, or [`parts.md`](../design/parts.md) stays authoritative for conventions and the spec covers only what a modeler needs. Both of those work. Having both documents state the rules independently is what does not, and that is the situation today.
