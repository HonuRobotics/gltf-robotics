...
- History in this repo
    - What this repo already decided: [`docs/design/parts.md`](https://honurobotics.github.io/bluerobotics_models/design/parts.html) (mesh conventions), [`docs/how-to/add-part.md`](https://honurobotics.github.io/bluerobotics_models/how-to/add-part.html) (the acceptance steps), the earlier README section "Accepting a new part" (commit `b5daead`, August 2026), and the sandbox findings carried over from the vrx4 branch.
...
**Origin: what we do now.** The written rule and the library disagree. [`docs/design/parts.md`](https://honurobotics.github.io/bluerobotics_models/design/parts.html) says the origin sits at the mesh centroid, the older README said the centroid and where the part would sensibly be mounted from, and the draft spec says it is the part author's choice. Measuring the delivered files against their own bounding boxes shows the library follows none of those three consistently, and instead follows a fourth rule nobody wrote down:
...
2. "`<part>.visual.glb`, next to `<part>.collision.stl`" and "Collision meshes are separate, simplified STL files." The visual name matches the repo convention. The collision statement does not: [`parts.md`](https://honurobotics.github.io/bluerobotics_models/design/parts.html) says collision is primitives stated in the macro, from the modeler's `model.sdf`, with a mesh only when a primitive will not do; one `.collision.stl` exists in the tree today against nine `model.sdf` files. Open. This is a pipeline decision (does the modeler still deliver `model.sdf`? do we accept STL envelopes?), and the spec must say which. See section 10, decision 1.
...
7. "Author the mesh in the part frame … Where the origin sits within the part is the part author's choice." Technically neutral: `attach` folds any origin into the mounting joint [S, `parts.xacro`]. It conflicts with [`parts.md`](https://honurobotics.github.io/bluerobotics_models/design/parts.html) rule 2, "origin at the mesh centroid", and the older README's "approximately at the centroid, in the place the part would sensibly be mounted from". Open. See section 10, decision 2.
...
The draft spec is not arriving into a vacuum. This repo already documents mesh conventions in [`docs/design/parts.md`](https://honurobotics.github.io/bluerobotics_models/design/parts.html), an acceptance procedure in [`docs/how-to/add-part.md`](https://honurobotics.github.io/bluerobotics_models/how-to/add-part.html), two glTF entries in the FAQ, and an older and more detailed acceptance checklist in the parts README as of commit `b5daead`. A new spec that contradicts those leaves two sources of truth, so this section reads the existing guidelines against what the audit established. It is deliberately about the existing documents, not the draft.

### 8.1 [`docs/design/parts.md`](https://honurobotics.github.io/bluerobotics_models/design/parts.html), the mesh conventions
...
The gap: [`parts.md`](https://honurobotics.github.io/bluerobotics_models/design/parts.html) says nothing about materials beyond "PBR materials embedded". Every material finding in this audit, the metalness defect included, has no home in the existing guidelines.

### 8.2 [`docs/how-to/add-part.md`](https://honurobotics.github.io/bluerobotics_models/how-to/add-part.html), the acceptance steps
...
Across [`parts.md`](https://honurobotics.github.io/bluerobotics_models/design/parts.html), the older README and the draft spec, the mesh origin is stated three ways: at the centroid, at the centroid and where the part would sensibly be mounted from, and wherever the part author chooses. The macro tolerates all three, so nothing has broken yet, and that is exactly why it has drifted. It needs one sentence in one place (section 10, decision 2).
...
11. Where the reconciliation in section 8 comes to rest. Either the draft spec absorbs the mesh conventions from [`parts.md`](https://honurobotics.github.io/bluerobotics_models/design/parts.html) and that file points at it, or [`parts.md`](https://honurobotics.github.io/bluerobotics_models/design/parts.html) stays authoritative for conventions and the spec covers only what a modeler needs. Both of those work. Having both documents state the rules independently is what does not, and that is the situation today.
