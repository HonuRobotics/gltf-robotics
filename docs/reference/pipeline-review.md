...
Working document as a part of the new modeling workflow. The intent is to review the background (what is our sources of truth for model file formats and review) and work throught the suggestions (see [`docs/reference/asset-spec.md`](../project/asset-spec-superseded.md)) on what our internal guidelines should be an what we should asess for each glTF/GLB file.  
...
The list below exposes the source documents used as references and input.  This is helpful for agents and for human review. 

- Exeternal specifications and guides
...
- Proposed assessment items from Carlos
    - [asset-spec.md](../project/asset-spec-superseded.md), the draft from Carlos, kept unmodified alongside this document. 
    - Two Claude artifacts Carlos linked, the "Parts Library Audit" (the 12 non-chassis parts) and the "BlueBoat Chassis Audit", both dated 2026-09-03 and both measured by a direct GLB parse against the draft spec. They could not be fetched from this session (public, non-member pages), so their text was pasted in and is reproduced verbatim alongside this document as [ASSET_AUDIT_PARTS_LIBRARY.md](../evidence/audit-parts-library.md) and [ASSET_AUDIT_BLUEBOAT_CHASSIS.md](../evidence/audit-blueboat-chassis.md); section 6 audits them. A third, the "BlueROV2 Chassis Audit", is referenced by both as a companion and has not been seen.
- History in this repo
    - What this repo already decided: [`docs/design/parts.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/design/parts.html) (mesh conventions), [`docs/how-to/add-part.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/how-to/add-part.html) (the acceptance steps), the earlier README section "Accepting a new part" (commit `b5daead`, August 2026), and the sandbox findings carried over from the vrx4 branch.
...
* Incorrect mental model equating classic (mesh+image) pair with glTF models.   
    - The current workflow was based on an incorrect idea that glb assets could be thought of as direct, drop-in replacement for a DAE Collada file referencing a paired PNG image (interpreted by the mesh as a texture).
    - Consequently, we largely ignored that the [glTF 2.0 specification](https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html) covers a much larger scope of 3D asset use cases than required by this project.    The spec for glTF assets includes a large variety arrays (e.g., scene, node, camera, animation, skin, etc. ) that are not pertinent to this use case.   
    - For this project we must be intentional about what subset of functionality is being used so that we (and our agents) don't naively apply the entire specification.  
    - Because glTF assets describe much more than the traditional (mesh, image) pair, we formulate [model-spec.md](../profile/profile.md) to describe both project conventions and constraints placed on models delivered in glTF format.
* No single source of "truth" with respect to visual rendering
    - Ryan is mainly looking at Blender, 
    - I'm mainly looking at Gazebo (and accidently tried an older version of Gazebo) and 
    - Carlos is using Gazebo and other viewers such as F3D, as well as Claude introspection of the file contents against [`asset-spec.md`](../project/asset-spec-superseded.md).
* Our consumers implement only a subset of glTF, so conforming (to glTF) and usable (in Gazebo and RVIZ) are different questions.
    - Gazebo and RViz each read part of the specification and quietly drop the rest. A file can be fully valid glTF and still fail in our stack, and no amount of validating against Khronos will catch it.
    - **Loud failures.** The case that proves it is already in this repo. The BlueROV2 chassis, as it stood before commit `01b05f9`, validated against the Khronos validator with zero errors and terminated RViz on load. The loader ([`assimp_loader.cpp`](https://github.com/ros2/rviz/blob/rolling/rviz_rendering/src/rviz_rendering/mesh_loader_helpers/assimp_loader.cpp#L313-L326)) asks every textured material for a diffuse texture; on a material whose only texture is a normal map the lookup fails, the path resolves to the mesh's own directory, and the resource retriever throws. The fix was to bake a 1x1 white base color into the file, which the specification makes visually free.
    - **Silent failures**, which are worse.  Here the file is valid, it loads without complaint, and it renders differently from what it describes, because the consumer reads the field and then ignores it. Nothing appears in a log, nothing fails a check, and the only symptom is a picture that is wrong in a way nobody can attribute to a cause. Gazebo drops `BLEND` alpha entirely, so a material authored translucent renders solid. It honors `doubleSided` only on a `MASK` material, so thin geometry vanishes when viewed from behind. It parses `KHR_texture_transform` and then discards it, so textures render in the wrong place on the surface. In each case the file is right, the renderer is quiet, and the result is wrong.
    - **Valid, honored, and still wrong for us.** A plastic hull declared fully metallic is exactly what the file asks for and exactly what the renderer delivers. No validator objects and no consumer misbehaves. The part is simply dark, and only someone who knows the boat is polyethylene can tell that it should not be.
    - Because a valid glTF asset may not be usable in Gazebo or RViz, [model-spec.md](../profile/profile.md) adds constraints that come from our targets rather than from the format, and say plainly which rules are ours and which are the standard's.
    - These subsets are also why the second issue above is not a matter of preference. The tools disagree because each implements a different part of the specification, so "it looks right in mine" is not evidence about anyone else's.

% CLAUDE: Add a short section here to summarize what gaps in our current workflow are filled by model-spec.md and the process it implies.  

## 0. Background

### 0.1 Sources of truth

Our goal is that our conventions, embodied in the model specification document ([model-spec.md](../profile/profile.md)), rest on external standards.  We do not want to stray from these specifications and conventions unless absolutely necessary; and where there is a departure from the underlying standards/specifications/conventions we state that explicilty and document it clearly in model-spec.md.   Where a standard genuinely says nothing, we say that too, rather than implying an authority that does not exist. If our conventions are silent on an topic, the default assumption is to be consistent with the underlying standard.  

Four different things get called "correct" in this work, and they do not agree. Every finding in this document is measured against one of them and says which.

Two of them are worth separating before reading the table, because conflating them is what produced the RViz crash above. Conformance asks whether the file is glTF, is judged by the Khronos validator, and is binary. Usability asks whether our consumers can actually load and render it, and is judged by running them. A file can be conforming and unusable, and it can be usable and still wrong. T1 answers conformance, T3 answers usability, and neither substitutes for the other.


|  | **Source of Truth** | **Current Assessment** | **Proposed Assessment** |
|---|---|---|---|
| **T1, Format**<br>Is it valid glTF? | The [glTF 2.0 specification](https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html), the Khronos extension registry (which documents official extensions to glTF) and the project-specific [model-spec.md](../profile/profile.md)| Currently we do not have a reviewed, verified method for checking the format.  Carlos' previous review revealed a number of possible (but unverified) inconsistencies-see [ASSET_AUDIT_PARTS_LIBRARY.md](../evidence/audit-parts-library.md) and [ASSET_AUDIT_BLUEBOAT_CHASSIS.md](../evidence/audit-blueboat-chassis.md).  | Two checks: first validate against the [Khronos glTF Validator](https://github.com/KhronosGroup/glTF-Validator), then against Honu's [model-spec.md](../profile/profile.md), which narrows the glTF spec to this project. Both can be formulated deterministic go/no-go tests, so both belong in CI beside the base color guard already in `test/test_parts.py`, with the workflow fetching the validator binary since it has no apt package. **Not agents:** a conformance gate has to give the same answer every time. Section 10, decision 7. Evidence tag V |
| **T2, Appearance**<br>Does it meet the visual requirement for this part? | The result of a conformant rendering tool, checked against the stated visual requirement for that part: how much geometric and texture detail its role in the project actually warrants | Two gaps, not one. No visual requirement is stated for any part, so there is nothing to check against and detail is whatever the modeler judged. And with no conformant renderer named, authors and reviewers use different tools that disagree: the modeler looks at Blender's viewport, which shows Blender's materials rather than the exported file, and nobody views the delivered file in a reference renderer | The modeler opens the exported file in the [Khronos glTF Sample Viewer](https://github.khronos.org/glTF-Sample-Viewer-Release/) and signs it off before delivery. Blender's viewport does not count. The viewer runs the Khronos validator inline, so one drag and drop answers T1 and T2 together. Evidence tag R |
| **T3, Targets**<br>Will our tools render it? | What our two consumers render: Gazebo (gz-sim 10, gz-rendering 10, Ogre-Next 2.3) and RViz (rviz_rendering 15.2), each implementing only a subset of T1 (sections 1.4 and 1.5) | Observation, on the versions we actually ship. Does the part render correctly  and load without error | `glb_probe` for what the loader built, which needs no GPU, then the parts world in Gazebo and the model in RViz. Evidence tags S and P |
| **T4, Budget**<br>Are the object sizes appropriate? | No external standard. glTF imposes no limit on triangles, texture size or file size, and neither does Gazebo, so the only authority is Honu's [model-spec.md](../profile/profile.md) | Nothing is checked. Measured for this review: files from 15 KB to 2.85 MB, 4.8 MB in total; triangles from 164 to 21,776, 43,866 in total; 33 embedded images, the largest 2.1 MB, and textures are 59 percent of all bytes stored | Thresholds stated in [model-spec.md](../profile/profile.md) and enforced by the lint, once section 10, decisions 4 and 5 fix the numbers. Texture resolution is the lever that matters, not triangle count |

% CLAUDE: I'm working in parallel to develop guidelines for this.  Make sure we finalize before publishing.
T4 is a different kind of row from the three above it. T1, T2 and T3 each measure against something outside this project: a published specification, a reference renderer, the tools we ship. T4 has no external authority at all, because glTF sets no limit on geometry or texture size and neither does Gazebo. We are the authority, so this row cannot be settled by reading a standard, only by deciding. That also makes it the row most likely to stay wrong for a long time without anyone noticing.

The measurement also answers the question that prompted the row. With textures embedded, they are the majority of every byte we store, so a budget on texture resolution buys far more than a budget on vertex count.

T2 and T4 are the same question asked from two ends, which is why neither can be settled alone. A budget is only defensible once the part's visual requirement is stated, because how much detail a part needs is what decides how much it may cost. The vehicles are looked at closely and driven, so they warrant the geometry and texture resolution we spend on them. A shore antenna is scenery, seen at distance and never interacted with, and does not. Today no part states its requirement, so every budget number would be uniform across parts that have nothing in common, and the base station antenna carries a 953 mm mast at the same nominal standard as a chassis.

What the numbers cannot tell us is what they ought to be. Every threshold in this row will be engineering judgment to begin with, not measurement: the earlier draft's 25,000 triangles was a guess, and any number we replace it with is a better-informed guess. The work that would justify a threshold is measuring how each factor actually affects load time, memory and frame rate in Gazebo on the hardware we run, separating the cost of geometry from the cost of texture memory. That is worth doing and is beyond the scope of this review.

The plan is therefore in two steps. Set provisional limits from judgment now, so a delivery has something to be checked against, and record them in [model-spec.md](../profile/profile.md) as guidelines with their basis stated as judgment rather than evidence. Then evaluate them against real performance when someone has time to measure, and revise the spec with what the measurement shows. Writing them into the spec is what makes the revision visible later; leaving them as habit would not.
...
- T2 is authoritative over intent. % CLAUDE: This is a gap in that the "intent" needs to be more clearly provided to the modeler.   Currently we do not give the modeller sufficient detail on our intent.   Need to add this as part of the worflow (maybe we also need a workflow document to specify the process - a companion to model-spec.md) We need to provide intention for each part being modelled.   Probably the best way to do this would be 1 - written description to modeler, 2 meeting to clarify intent, 3 final review of written part-by-part priorities.   This all happens before modeler starts working and informs the visual assessment.
The modeler works in Blender's shader graph and viewport, which render Blender's material, not the file. The exporter is a translation, and things are lost or defaulted in it (metallic factor, image format, alpha mode, tangents). Blender is therefore not a viewer of the delivery at all. The modeler-side check of the file is to open the exported GLB in the Sample Viewer % CLAUDE: include link to online viewer
 Third-party viewers (the Babylon sandbox, three.js viewers, f3d) each implement a different subset of the glTF specification with their own defaults.
- T3 is authoritative over acceptance. A part is accepted when it renders as intended in Gazebo and loads in RViz, because that is what ships. A T3 gap is our constraint to document.
- Gazebo is not a reference for anything. It has not implemented the whole of T1 (section 1.4 lists the fields and extensions it drops or alters). "It looks right in Gazebo" is necessary and not sufficient: a file can look right in Gazebo because a defect happens to be dropped (the BlueBoat's BLEND flag), and a conformant file can look wrong in Gazebo (any BLEND glass).

Here are common types of failures  % CLAUDE: Take a another pass here and clarify.   Just to columns: Class and Description (what fails)
| Class | Fails | Who fixes it | In the current deliveries |
|---|---|---|---|
| Invalid | T1: the validator reports an error % CLAUDE: what do you mean the validator?   How are we going to check that we meet both the glTF and model-md specifications?   What are the steps to run that validation?| The modeler, at export | None. All 15 files validate with zero errors (section 2.6). |
| Unintended | T2: valid (both ), but not what the modeler meant | The modeler, at source | Metalness 1.0 on plastic hulls; 22 percent of the BlueROV2 chassis without a material; BLEND on an opaque hull |
...
Every requirement % CLAUDE: where are the requiements listed?
gets three separate answers, because they fail independently:

| Question | Meaning |
|---|---|
| Technically correct? | Does the toolchain we ship % CLAUDE: "toolchain we ship" is vague.
actually behave the way the requirement's rationale says? Judged against the versions installed in the drydock container and listed in section 1.2 below. |
...
* Images either sit in the BIN chunk, referenced by a `bufferView`, or are external files referenced by a `uri`. The format allows both and both consumers (Gazebo and RVIZ) load either, so which one we require is a project constraint rather than a format question. Every current delivery embeds (vice reference external file). Section 10, decision 16 sets out the trade-off: embedding gives one artifact that cannot arrive incomplete, while external files let a texture be re-authored and reviewed without re-exporting the geometry.
...
**Origin: what we do now.** The written rule and the library disagree. [`docs/design/parts.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/design/parts.html) says the origin sits at the mesh centroid, the older README said the centroid and where the part would sensibly be mounted from, and the draft spec says it is the part author's choice. Measuring the delivered files against their own bounding boxes shows the library follows none of those three consistently, and instead follows a fourth rule nobody wrote down:
...
A 90-line program linked against the installed gz-common 7 that loads a mesh through `MeshManager` exactly as Gazebo does and prints the submeshes (name, counts, UV sets, material index) and every material (diffuse RGBA, transparency, alpha-from-texture, threshold, two-sided, emissive, and each PBR map with its decoded size). Loader errors and debug lines print inline. It lives in `probe/glb_probe/` for now; it should move into `bluerobotics_parts/scripts` (or a `gz`-style tool in `maritime-workspace`) once the acceptance list references it.

```bash
# build once, inside drydock
cd probe/glb_probe && mkdir -p build && cd build
...
probe/glb_probe/build/glb_probe <part>/<part>.visual.glb
```
...
This section audits Carlos's draft as delivered, kept unmodified at [asset-spec.md](../project/asset-spec-superseded.md). It has since been superseded by [model-spec.md](../profile/profile.md), and the findings below are what shaped it.
...
2. "`<part>.visual.glb`, next to `<part>.collision.stl`" and "Collision meshes are separate, simplified STL files." The visual name matches the repo convention. The collision statement does not: [`parts.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/design/parts.html) says collision is primitives stated in the macro, from the modeler's `model.sdf`, with a mesh only when a primitive will not do; one `.collision.stl` exists in the tree today against nine `model.sdf` files. Open. This is a pipeline decision (does the modeler still deliver `model.sdf`? do we accept STL envelopes?), and the spec must say which. See section 10, decision 1.
...
The draft spec is not arriving into a vacuum. This repo already documents mesh conventions in [`docs/design/parts.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/design/parts.html), an acceptance procedure in [`docs/how-to/add-part.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/how-to/add-part.html), two glTF entries in the FAQ, and an older and more detailed acceptance checklist in the parts README as of commit `b5daead`. A new spec that contradicts those leaves two sources of truth, so this section reads the existing guidelines against what the audit established. It is deliberately about the existing documents, not the draft.

### 8.1 [`docs/design/parts.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/design/parts.html), the mesh conventions
...
The gap: [`parts.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/design/parts.html) says nothing about materials beyond "PBR materials embedded". Every material finding in this audit, the metalness defect included, has no home in the existing guidelines.

### 8.2 [`docs/how-to/add-part.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/how-to/add-part.html), the acceptance steps
...
Across [`parts.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/design/parts.html), the older README and the draft spec, the mesh origin is stated three ways: at the centroid, at the centroid and where the part would sensibly be mounted from, and wherever the part author chooses. The macro tolerates all three, so nothing has broken yet, and that is exactly why it has drifted. It needs one sentence in one place (section 10, decision 2).
...
11. Where the reconciliation in section 8 comes to rest. Either the draft spec absorbs the mesh conventions from [`parts.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/design/parts.html) and that file points at it, or [`parts.md`](https://honurobotics.github.io/bluerobotics_models/lyrical/design/parts.html) stays authoritative for conventions and the spec covers only what a modeler needs. Both of those work. Having both documents state the rules independently is what does not, and that is the situation today.
...
18. Whether every part declares a visual requirement, and in what form. This is the input T2 is currently missing and the input T4 needs before any budget number is defensible. The requirement is qualitative: how much geometric and texture detail this part's role in the project warrants. One workable form is a small number of named tiers, so that a delivery can be checked against the tier rather than against taste. A first cut would be a functional tier for the vehicles and the parts fitted to them, which are seen close and driven; an accessory tier for sensors and brackets; and a scenery tier for decorative items such as shore antennae and flags, seen at distance and never interacted with. Whatever the form, it has to be recorded per part, since a reviewer cannot otherwise tell over-modeling from under-modeling, and both waste something.
